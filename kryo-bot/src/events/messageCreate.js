...
module.exports = {
  ...
  async execute(message) {
    try {
      if (!message.guild || message.author.bot || !message.member) return Promise.resolve();
      ...
      if (!cfg.pingRole) return Promise.resolve();
      if (!messageTargetsProtected(message, cfg)) return Promise.resolve();

      // Someone protected pinging another protected person/role, or pinging themselves, isn't a violation
      if (message.member.roles.cache.has(cfg.pingRole)) return Promise.resolve();

      // Check if user's role is allowed to ping without warnings
      if (storage.isPingAllowed(message.guild.id, message.member)) {
        return Promise.resolve(); // They're allowed, do nothing
      }
